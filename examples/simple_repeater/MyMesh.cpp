...
bool MyMesh::allowPacketForward(const mesh::Packet *packet) {
  ...
  if (packet->isRouteFlood() && !prox_suppress.allowForward(millis())) {
    MESH_DEBUG_PRINTLN("allowPacketForward: flood suppressed by prox");
    return false;
  }
  return true;
}
...
mesh::DispatcherAction MyMesh::onRecvPacket(mesh::Packet* pkt) {
  ...
  // Observe proximity before dedup/forward so listed upstream repeaters still
  // arm the suppress timer even when we already saw the packet from the origin.
  prox_suppress.observePacket(pkt, pkt->_snr, millis());
...
void MyMesh::applyProxFromPrefs() {
  char err[48];
  prox_suppress.setEnabled(_prefs.prox_enabled != 0);
  prox_suppress.setSnrThreshold(_prefs.prox_snr);
  prox_suppress.setHoldSecs(_prefs.prox_hold);
  if (!prox_suppress.parseAndSetList(_prefs.prox_list, err, sizeof(err))) {
    MESH_DEBUG_PRINTLN("prox: failed to load list: %s", err);
  }
}

void MyMesh::handleProxCommand(char *command, char *reply) {
  reply[0] = 0;

  if (memcmp(command, "get prox", 8) == 0 && (command[8] == 0 || command[8] == ' ')) {
    prox_suppress.formatStatus(_prefs.prox_list, reply, 160, millis());
    return;
  }

  if (memcmp(command, "set prox", 8) != 0) {
    strcpy(reply, "Err - bad prox command");
    return;
  }
  ...
  if (strcmp(args, "on") == 0) {
    _prefs.prox_enabled = 1;
    prox_suppress.setEnabled(true);
    savePrefs();
    strcpy(reply, "OK");
    return;
  }
  if (strcmp(args, "off") == 0) {
    _prefs.prox_enabled = 0;
    prox_suppress.setEnabled(false);
    savePrefs();
    strcpy(reply, "OK");
    return;
  }
  if (strncmp(args, ".snr ", 5) == 0) {
    int snr = atoi(args + 5);
    if (snr < -128 || snr > 127) {
      strcpy(reply, "Err - bad snr");
      return;
    }
    _prefs.prox_snr = (int8_t)snr;
    prox_suppress.setSnrThreshold(_prefs.prox_snr);
    savePrefs();
    strcpy(reply, "OK");
    return;
  }
  if (strncmp(args, ".hold ", 6) == 0) {
    int hold = atoi(args + 6);
    if (hold < 0 || hold > 65535) {
      strcpy(reply, "Err - bad hold");
      return;
    }
    _prefs.prox_hold = (uint16_t)hold;
    prox_suppress.setHoldSecs(_prefs.prox_hold);
    savePrefs();
    strcpy(reply, "OK");
    return;
  }
  if (strncmp(args, ".list ", 6) == 0) {
    const char* list = args + 6;
    while (*list == ' ') list++;
    char err[48];
    if (!prox_suppress.parseAndSetList(list, err, sizeof(err))) {
      strcpy(reply, err);
      return;
    }
    StrHelper::strncpy(_prefs.prox_list, list, sizeof(_prefs.prox_list));
    savePrefs();
    strcpy(reply, "OK");
    return;
  }
