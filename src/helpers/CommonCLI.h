...
public:
  ...
  uint8_t extra_sf[4];
  uint8_t prox_enabled = 0;
  int8_t prox_snr = 8;
  uint16_t prox_hold = 600;
  char prox_list[MAX_PROX_LIST_LEN];
...
private:
  ...
  protected:
    void structure() override {
      ...
      def("prox_en", _parent->prox_enabled);
      def("prox_snr", _parent->prox_snr);
      def("prox_hold", _parent->prox_hold);
      def("prox_list", _parent->prox_list, sizeof(_parent->prox_list));
    }
  public:
